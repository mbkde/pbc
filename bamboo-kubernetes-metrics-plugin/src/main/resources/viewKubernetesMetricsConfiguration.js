 ...
 */
   var restEndpoint = AJS.contextPath() + "/rest/pbc-kubernetes-metrics/latest/";
    ...
    function processConfig(response) {
        updateStatus("");
        AJS.$("#prometheusUrl").val(response.prometheusUrl);
    }

    function setRemoteConfig() {
        var config = {};
        config.prometheusUrl = AJS.$("#prometheusUrl").val().trim();
