...
AJS.$(document).ready(function () {
    ...
    if (isRequirements) {
        insertionQ('table.requirements-table').every(function (element) {
            ...
            $("td:eq(4)", row).replaceWith("<td><span class='aui-lozenge aui-lozenge-current aui-lozenge-subtle' original-title='Only Per-build Container Docker agents can build this job'>Per-build Container Docker Agents only</span></td>");
            if (row.length) {
                $('div.requirements-info').replaceWith(
                    "<div class='aui-message aui-message-warning warning'><p class='title'>" + 
                    "<strong>This job is built by Per-build Container Docker Agents.</strong></p>" +
