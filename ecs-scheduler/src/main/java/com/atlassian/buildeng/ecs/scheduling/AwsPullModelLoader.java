...
public final class AwsPullModelLoader implements ModelLoader {
    ...
    // 5 might be too radical, but I haven't found any stats on what this the mean/average or 95percentile time for ec2
    // instance startup
    ...
    // AWS Console - ASG activity hisotry - Start 2016 December 16 15:44:36 UTC+11 -> End 2016 December 16 15:45:09
    // UTC+11
    ...
    @Inject
    public AwsPullModelLoader(
            SchedulerBackend schedulerBackend, EventPublisher eventPublisher, ECSConfiguration globalConfiguration) {
    ...
    DockerHosts loadHosts(String cluster, AutoScalingGroup asg) throws ECSException {
        ...
        Map<String, ContainerInstance> containerInstances =
                schedulerBackend.getClusterContainerInstances(cluster).stream()
        ...
        Set<String> asgInstances =
                asg.getInstances().stream().map(x -> x.getInstanceId()).collect(Collectors.toSet());
        Set<String> allIds = new HashSet<>();
        allIds.addAll(asgInstances);
        allIds.addAll(containerInstances.keySet());
        Map<String, Instance> instances = schedulerBackend.getInstances(allIds).stream()
        ...
        if (!lonelyAsgInstances.isEmpty()) {
            lonelyAsgInstances.stream()
                    .filter((String t) -> {
                        ...
                        if (ec2 != null) {
                            final long lifespan =
                                    new Date().getTime() - ec2.getLaunchTime().getTime();
                            return Duration.ofMinutes(ASG_MISSING_IN_CLUSTER_GRACE_PERIOD)
                                                    .toMillis()
                                            < lifespan
            ...
            logger.warn(
                    "Scheduler got different lengths for instances ({}) and container instances ({})",
                    asgInstances.size(),
